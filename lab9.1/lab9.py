    ...
    for sign in [1, -1]:
        ...
        if x1s_sq > 0:
            for s2 in [1, -1]:
                ...
                x2s = 1 / x1s
                lbl = 'Точні розв\'язки' if first_label else ""
                plt.plot(x1s, x2s, 'k*', markersize=12, label=lbl)
                first_label = False
...
def run_test():
    """Тест методу Хука-Джівса на функції Розенброка."""
    print("\n" + "="*60)
...
def run_main():
    ...
    X0    = [1.5, 0.5]          # початкове наближення
    delta = [0.5, 0.5]          # початковий крок
    q     = 0.5
    p     = 2.0
    eps1  = 1e-6
    eps2  = 1e-10
    ...
    print("\n" + "="*60)
    print("ПУНКТ 5: Збереження координат траєкторії та графік")
    print("="*60)
    ...
    print(f"  Файл збережено: trajectory.txt")
    ...
    # Малюємо лінії пошуку
    plt.plot(traj_arr[:, 0], traj_arr[:, 1], 'w.-', linewidth=1.0,
             markersize=4, alpha=0.8, label='Траєкторія')
    ...
    plt.show()  # Відкриває вікно з графіком траєкторії
    plt.close()
